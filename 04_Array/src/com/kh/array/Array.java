...
public class Array {
	 ...
	 * 
	 * 자바러는 언어 자체에 내장된 유일한 자료구조
	 * 
	 * => 배열의 각 인덱스에 실제 값이 담김. index는 '0'부터 시작한다.
	 * 
	 */
	public void method1() {
		// 임의의 정수 6개 선언하고 초기화를 진행한다
		System.out.println("법을 잘 키십시다.");
		int num1 = 12;
		int num2 = 13;
		int num3 = 77;
		int num4 = 7;
		 ...
		 * 배열 선언 방법
		 * 
		 * 1) 자료형 배열식별자[]; 2) 자료형 [] 배열식별자; --> 요 방법 사용할 것!!!!
		 */
		...
		/*
		 * 배열에 몇 개의 값이 들어갈지 배열의 크기를 정해주는 과정 지정한 개수만큼 값이 들억라 공간이 할당됨
		 * 
		 * [ 표현법 ]
		 * 
	...
	public void methodA() {
		...
		integers[0] = 1;
		/*
		 * 기본 자료형 : boolean, char, byte, short, int, long, float, double =>변수 공간에 실제 값을
		 * 바로 담을 수 있음 : (지역)변수
		 * 
		 * 참조 자료형 : boolean[], char[], byte[], int[] ... String =>주소값을 담고 있는 변수 : 참조
		 * 변수(레퍼런스)
		 * 
		 */
		...
		// System.out.println("종로점 주소 : " + KH정보교육원종로지원[0]);
		...
		// 참고자료형에서는 ==(동등비교 연산자) 사용시
		// 주소값을 비교하기 때문에 원하는 결과를 얻을 수 있음
		// 문자열 .eqals()

		// 해시코드 : 주소값을 10진수(int형태)형태로 나타낸 것
		// System.out.println(number2.hasCode());
		// System.out.println(number2.hasCode());
	...
	public void lotto() {
		 ...
		 * (int)(Math.random() * 45) +1; lottoNumbers[2] = (int)(Math.random() * 45) +1;
		 * lottoNumbers[3] = (int)(Math.random() * 45) +1; lottoNumbers[4] =
		 * (int)(Math.random() * 45) +1; lottoNumbers[5] = (int)(Math.random() * 45) +1;
		 */
	...
	public void methodC() {
		...
		// 1) 사용자로부터 값을 입력을 받아서 배열의 각 인덱스에 대입
		Scanner sc = new Scanner(System.in); // 입력도구
		int[] nums = new int[3]; // 저장소

		// 배열은 물리적인구조와 논리적인 구조가 동일합니다.
		/*
		 * System.out.print("첫 번째 정수를 입력하세요. >"); nums[0] = sc.nextInt();
		 * 
		 * System.out.print("두 번째 정수를 입력하세요. >"); nums[1] = sc.nextInt();
		 * 
		 * System.out.print("세 번째 정수를 입력하세요. >"); nums[2] = sc.nextInt();
		 * 
		 * System.out.printf("nums라는 배열의 인덱스에 대입한 값 : %d, %d, %d", nums[0] ,nums[1]
		 * ,nums [2]);
		 */
	...
	public void methodD() {
		 ...
		 * 
		 * 기존배열식별자에 할당만 새롭게 한다면 => 기존 참고하고 있던 연결이 끊기고 새로운 배열을 참조함
		 * 
		 * 기본값
		 * 
		 * 정수 = 0 실수 = 0.0 문자 = ''
	...
	public void methodF() {
		...
		// 얕은 복사 => 주소값을 대입하는 것 이기 떄문에 가리키고 있는 대상이 같다.
