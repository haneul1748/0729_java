...
public class Run {
	...
	public static void main(String[] args) {
		...
		p1.printParent();
		// p1.printChild(); 자식이 가지고 있는 멤버는 부모가 사용할 수 없음
		...
		// 3. 부모 타입 자료형으로 자식객체를 다룸 ==> 다형성 적용
		Parent p2 = /* (Parent) */new Child1(); // 우리눈에는 보이지않지만 자료형변화가된거임
		// Parent p3 = new Test();
		p2.printParent();
		// p2.printChild1();
		...
		for (int i = 0; i < arr.length; i++) {
			/*
			 * if(i == 0 || i == 3) { ((Child1)arr[i]).printChild1(); } else {
			 * ((Child2)arr[i]).printChild2(); }
			 * 
			 * 
			 * if(arr[i] instanceof Child1) { // instanceof => true / false // 현재 참조변수가
			 * 실질적으로 어떤 클래스타입을 참조하고 있는지 확인할 때 사용 ((Child1)arr[i]).printChild1(); } else {
			 * ((Child2)arr[i]).printChild2(); } }
			 */
			...
			/*
			 * 동적바인딩 : 프로그램 "샐행 전" 컴파일 시점에 정적바인딩 수행(자룧의 메소드를 가리킴)
			 * 			실질적으로 참조하고 있는 객체가 해당 메소드를 오버라이딩 했다면
			 * 			프로그램 "실행 시점" 동적으로 자식클래스의 오버라이딩된 메소드를 실행
			 * 					(Runtime)
			 * 
			 * 
			 * 다향성 : 부모클래스 자료형으로 여러 자식클래스를 다루는 기술
			 * 
